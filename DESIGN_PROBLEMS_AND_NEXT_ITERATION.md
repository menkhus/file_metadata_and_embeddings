...
## Problem 5: Fixed-Size Chunks Split Mid-Sentence

**Discovered:** 2026-03-09 during recall agent integration.

### What was found

The `text_chunks` table contains 392,867 chunks generated by a fixed-character-count
splitter (~1000 chars). Inspection of a representative file showed chunks cut
mid-sentence and mid-word with no awareness of paragraph, sentence, or semantic
boundaries.

Example (design-cognitive-housekeeping.md):
```
chunk 0 ends:   ...{todo, accepted: [...
chunk 1 starts: d\n  → predict informationally dependent documents...
```

Same sentence, split across chunks at a character boundary.

### Why it matters

- A retrieved chunk may be half a thought — the context passed to an LLM or shown
  to a user is truncated mid-idea
- Term-overlap scoring for chunk selection is unreliable when the matched term may
  be in the adjacent chunk, not the one returned
- Embedding quality is degraded — embedding a partial sentence gives a less
  meaningful vector than embedding a complete semantic unit

### Current workaround (in recall agent)

`~/AI/actions/recall.py` fetches the best-matching chunk plus its neighbors
(chunk_index - 1 and chunk_index + 1) to ensure complete context around any
boundary. This is a retrieval-side patch, not a fix.

### The correct fix — already written, not yet deployed

`chunking_refactor.py` contains `AIOptimizedChunker`, which does exactly what
is needed:
- Splits prose at `\n\n` paragraph boundaries
- Groups paragraphs until ~800 chars
- Splits oversized paragraphs at sentence boundaries (`[.!?]+\s+`)
- Splits code at line boundaries
- Records chunk strategy, position, and adjacency metadata

`chunk_db_integration.py` contains `ChunkDatabase`, which stores envelopes
from `AIOptimizedChunker` into `text_chunks_v2` with a `chunks_fts` FTS5 index
for chunk-level BM25 search. `schema_refactor.sql` creates these tables.

**None of this is wired into the live indexer.** The active code path is
`file_metadata_content.py:chunk_text()` (line 509), which uses a fixed 1000-char
window with 200-char overlap.

### What needs to happen (deployment, not coding)

```
1. Run schema_refactor.sql against ~/data/file_metadata.sqlite3
       → creates text_chunks_v2 and chunks_fts tables

2. Run re-indexer using AIOptimizedChunker across all indexed files
       → populates text_chunks_v2 (replaces 392K bad chunks)

3. Update mcp_server_fixed.py get_file_chunks() to read from text_chunks_v2
       → one query change, schema is compatible

4. Update full_text_search() or add chunk_search() that queries chunks_fts
       → enables chunk-level BM25, currently only file-level exists

5. Drop text_chunks after validation
```

### Impact

Re-chunking is a prerequisite for the PPMI/LSA extension (groundwork Phase 1),
since PPMI co-occurrence quality depends on complete semantic units as the
token stream input, not arbitrary character windows.

The coding is done. This is a migration task.
